<template>
  <el-container>
    <el-main>
      <el-alert title="编辑新闻" type="info" center show-icon :closable="false"></el-alert>
  ...
  methods: {
    getNews (id) {
      this.$store.state.cloud.callFunction({
        ...
        this.newsForm = res.result.data
        this.getTempFile(res.result.data.cover)
      })
    },
    change (file) {
    ...
    getTempFile (url) {
      this.$store.state.cloud.callFunction({
        ...
        this.$set(this.fileList, 0, { url: res.result[0].tempFileURL })
      })
    }
  },
  created () {
